<template>
  <div class="demo">
    <div>
      <div>
        <h4>tag</h4>
        <bk-select class="bk-select" v-model="selectedValue" multiple-mode="tag" collapse-tags filterable multiple
          show-select-all>
          <bk-option v-for="(item, index) in datasource" :id="item.value" :key="index" :name="item.label" />
      ...
      <div>
        <h4>trigger</h4>
        <bk-select class="bk-select" v-model="selectedValue" :input-search="false" filterable multiple>
      ...
      <div>
        <h4>extension</h4>
        <bk-select class="bk-select" v-model="selectedValue" filterable @toggle="handleToggle">
    ...
    <div>
      ...
      <div>
        <h4>prefix(属性)</h4>
        <bk-select class="bk-select" v-model="selectedValue" :input-search="false" prefix="运动选项" filterable multiple>
          <bk-option v-for="(item, index) in datasource" :id="item.value" :key="index" :name="item.label" />
        </bk-select>
      </div>
      <div>
        <h4>prefix(属性)</h4>
        <bk-select class="bk-select" v-model="selectedValue" :auto-height="false" multiple-mode="tag" prefix="运动选项"
          collapse-tags filterable multiple show-select-all>
          <bk-option v-for="(item, index) in datasource" :id="item.value" :key="index" :name="item.label" />
        </bk-select>
      </div>
    </div>
    <div>
      <div>
        <h4>optionRender</h4>
        只用使用list 列表才会有效。
        <bk-select class="bk-select" v-model="selectedValue" :input-search="false" :list="datasource"
          display-key="label" id-key="value" enable-virtual-render filterable multiple>
      ...
      <div>
        <h4>optionRender</h4>
        <bk-select class="bk-select" v-model="selectedValue2" :input-search="false" :list="datasource"
          display-key="label" id-key="value" filterable>
          <template #optionRender="{ item }">
            <span>{{ item.label }}</span>
      ...
      <div>
        <h4>option</h4>
        <bk-select class="bk-select" v-model="selectedValue2" :input-search="false" filterable multiple>
          <bk-option v-for="(item, index) in datasource" :id="item.value" :key="index" :name="item.label">
            <div style="width: 100%" @click.prevent.stop>
              <bk-input v-if="editOption === item.value" style="width: 100px" @enter="addOptions" />
              <div v-else @click="handleEditOption(item.value)">
                {{ item.label }}
              </div>
            </div>
          </bk-option>
